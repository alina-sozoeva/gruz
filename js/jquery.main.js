...
jQuery(function () {
	...
	jQuery(document).click(function (e) {
		...
		if (jQuery('body').hasClass('nav-active')) {
			if (
				!jQuery(target).hasClass('nav-opener') &&
				!jQuery(target).parents('.main-nav').length
			) {
...
const options = {
  // Определите ваши параметры модального окна здесь
  // Например:
  keyboard: false, // Отключить закрытие модального окна с клавиатуры
};
// const exampleModalAlternative = new bootstrap.Modal('#exampleModal', options);
...
function initSlickCarousel() {
	...
	jQuery('.trending-slider').slick({
		...
		arrows: true,
		prevArrow:
			'<button class="slick-prev"><i class="icon icon-arrow-right1"></i></button>',
		nextArrow:
			'<button class="slick-next"><i class="icon icon-arrow-right1"></i></button>',
		dots: true,
		focusOnSelect: false,
		infinite: true,
		autoplay: false,
...
jQuery(window).on('resize', function () {
	...
	let splides = document.querySelectorAll(".js-splide"); if (splides.length){' '}
